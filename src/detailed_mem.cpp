...
uint64_t MemChannelBase::LatencySimulate(Address lineAddr, uint64_t arrivalCycle, uint64_t lastPhaseCycle, MemAccessType type) {
    ...
    //info("Addr:%lu, StartCycle:%lu\n",lineAddr,arrivalCycle);
    return latency;
}
...
void MemChannelBase::AddressMap(Address addr, uint32_t& row, uint32_t& col, uint32_t& rank, uint32_t& bank) {
    ...
    row = addr;
	//printf(", Row:%d, Rank:%d, Bank:%d, Channel:%d, Col:%d\n",row,rank,bank,chnl,col);
...
bool MemSchedulerDefault::CheckSetEvent(MemAccessEventBase* ev) {
    ...
    for(it = wrQueue.begin(); it != wrQueue.end(); it++) {
        if (it->second == ev->getAddr()) {
            if (ev->getType() == WRITE) {
                wrQueue.erase(it);
	//saurav	
	wrQueue.push_back(MemSchedQueueElem(ev,ev->getAddr()));
               // wrQueue.push_back();
            }
            wrQueueHits++;
            return true;
	...
	//saurav        
        wrDoneQueue.push_back(MemSchedQueueElem(ev,ev->getAddr()));
...
bool MemSchedulerDefault::GetEvent(MemAccessEventBase*& ev, Address& addr, MemAccessType& type) {
    ...
    ev = it->first;
            ...
            type = WRITE;
            wrQueue.erase(it);
            wrDoneQueue.push_back(MemSchedQueueElem(ev, addr));
...
void MemControllerBase::enqueue(MemAccessEventBase* ev, uint64_t cycle) {
    ...
    //printf("Channel:%d, wrDoneQueueHits:%d, wrQueueHits:%d\n",channel,sches[channel]->wrDoneQueueHits,sches[channel]->wrQueueHits);
    if (ev->getType() == READ) {
        if (bRet)
...
uint64_t MemControllerBase::access(MemReq& req) {
    ...
    if ((req.type != PUTS) && zinfo->eventRecorders[req.srcId]) {
        ...
        MemAccessEventBase* memEv =
            ...
            MemAccessEventBase(this, accessType, addr, domain, preDelay[accessType], postDelay[accessType],req.srcId);
	//printf("CoreID:%d\n",memEv->getsrc());
