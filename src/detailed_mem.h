...
//	typedef std::pair<MemSchedQueueEle,uint32_t> MemSchedQueueElem;
        uint32_t id;
        MemParam* mParam;
        MemChannelBase* mChnl;

    public:
        uint32_t wrDoneQueueHits;
        uint32_t wrQueueHits;
...
class MemAccessEventBase : public TimingEvent {
    ...
    public:
        MemAccessEventBase(MemControllerBase* _dram, MemAccessType _type, Address _addr, int32_t domain, uint32_t preDelay, uint32_t postDelay,uint32_t _srcId)
            : TimingEvent(preDelay, postDelay, domain), dram(_dram), type(_type), addr(_addr), srcId(_srcId) {}
